...
const Ball = (props) => {
  const [products, setProducts] = useState({})
    ...
    let balls = {}
       ...
       {
              if(item.title in balls)
              {
                     if(!balls[item.title].color.includes(item.color) &&  item.availableQty>0)
                     {
                            balls[item.title].color.push(item.color)
                     }
                     if(!balls[item.title].size.includes(item.size) &&  item.availableQty>0)
                     {
                            balls[item.title].size.push(item.size)
                     }
              }
              else
              {
                     balls[item.title] = JSON.parse(JSON.stringify(item))
                     if(item.availableQty>0)
                     {
                            balls[item.title].color = [item.color]
                            balls[item.title].size = [item.size]
                     }
              }
       }
    setProducts(balls);
  ...
  <div className="container px-5 py-24 mx-auto">
    <div className="flex flex-wrap -m-4 justify-center">
      {Object.keys(products).map((items) =>(
       <Link key={products[items]._id} to={`/ball/${products[items].slug}`} onClick={props.clLink("ball")} className="nav-link lg:w-1/5 md:w-1/2 p-4 w-full  shadow-lg m-3">
        <div>
